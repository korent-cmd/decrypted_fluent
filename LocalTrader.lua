...
local CONFIG = {
	...
	HubPollInterval = 3,
	-- Legacy local-order settings are retained for backwards compatibility.
...
local function hubRequest(method, path, body)
	...
	-- Prefer the executor request API because it reliably supports custom headers.
	...
	-- Fallback for runtimes exposing Roblox's RequestAsync.
...
local function readExternalOrder()
	...
	local customer = tostring(request.customer or "")
	local item = tostring(request.item or "")
	...
	return {
		version = 4,
		phase = "TRADING_ROUTE",
		status = tostring(request.status or "ASSIGNED"),
		order_id = requestId,
		request_id = requestId,
		customer = customer,
		item = item,
		quantity = math.max(1, tonumber(request.quantity) or 1),
		account_id = tostring(data.account_id or HubConfig.accountId or ""),
	}
end

local function writeExternalOrder(order, phase, status)
	-- The Hub is the source of truth now. LocalTrader only reports the
	-- verified completion; rw.py then sees FARMING and relaunches Roblox.
				...
				local after = type(inventoryAfter) == "table" and inventoryAmount(inventoryAfter, session.requestedId) or nil
					...
					setState("COOLDOWN", "trade verified; preparing farming handoff")
						...
						if session.externalOrder then
							local reported = writeExternalOrder(session.externalOrder, "FARMING", "COMPLETED")
							if not reported then
								writeLog("trade verified locally, but Hub completion report failed; retrying")
								setState("VERIFYING_INVENTORY", "waiting for Hub acknowledgement")
								return
							end
						end
...
-- Keep LocalTrader passive in farming, but poll the Hub so a trade order is
-- picked up immediately if the account is already in the trading server.
task.spawn(function()
	while true do
